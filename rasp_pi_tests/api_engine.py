...
class API_Engine(Thread):
    ...
    def do_request(self, type, url, body = {}):
        ...
        if self.instance.send:
            if type == "put":
                response = requests.put(url=url, json=body, verify='./certificate.pem')
            elif type == "post":
                response = requests.post(url=url, json=body, verify='./certificate.pem')
            elif type == "delete":
                if body:
                    response = requests.delete(url=url, json=body, verify='./certificate.pem')
                else:
                    response = requests.delete(url=url, verify='./certificate.pem')
            elif type == "get":
                response = requests.get(url=url, verify='./certificate.pem')
        return response
